<x-jet-action-section>
    <x-slot name="title">
        {{ __('Sesiones de navegacion') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Muestra los dispositivos que estamos utilizando actualmente') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si es necesario puede cerrar las sesiones de los otros navegadores en los cuales no se este utilizando') }}
        </div>
                        ...
                        <div class="ml-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('Ultimo activo') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('CERRAR LA SESIÓN DE LOS OTROS DISPOSITIVOS') }}
            </x-jet-button>
